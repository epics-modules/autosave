...
static int tryCopy(char *source, char *dest)
{
	int 	source_fd, dest_fd, bytes, b;
	char	buffer[120];

	if ((source_fd = open(source, O_RDONLY, 0)) == NULL) {
		printf("reboot_restore:tryCopy: Can't open file %s\n", source);
		return(-1);
	}
	if ((dest_fd = open(dest,O_CREAT | O_RDWR, sr_umask)) == NULL) {
		printf("reboot_restore:tryCopy: Can't open file %s\n", dest);
		return(-1);
	}
	while ((bytes = read(source_fd, buffer, 120))) {
		b = write(dest_fd, buffer, bytes);
	}
	close(source_fd);
	close(dest_fd);
	return(0);
